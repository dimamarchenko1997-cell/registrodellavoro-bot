...
async def remind_ingresso():
    ...
    try:
        sheet = get_sheet("Registro")
        rows = sheet.get_all_values()
        all_users = set(row[1] for row in rows[1:])  # Tutti utenti unici dal sheet
        ...
        for user_str in missing_users:
            user_id = int(user_str.split(" | ")[1])  # Estrai ID da "Nome | ID"
            await send_reminder(user_id, "Ricorda di registrare l'ingresso entro le 9:00!")
    except Exception as e:
...
async def remind_uscita():
    ...
    try:
        ...
        for user_str in missing_exit:
            ...
            await send_reminder(user_id, "Non dimenticare di registrare l'uscita!")
    except Exception as e:
...
async def on_startup():
    ...
    for day in days:
        getattr(aioschedule.every(), day).at("09:00").do(remind_ingresso)
