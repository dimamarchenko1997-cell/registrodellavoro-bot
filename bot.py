...
# =========================================================
# === NUOVO BLOCCO: SCANSIONE ID BUS E MATRICOLE (foto + testo)
# =========================================================
...
# Dispositivi nell'ordine richiesto
DEVICE_LIST = [
...
async def scan_start(message: Message, state: FSMContext):
    """
    Inizia il flusso: chiede manualmente l'ID BUS.
    """
...
async def scan_button(message: Message, state: FSMContext):
    # se preferisci usare il bottone dal menu
...
async def process_bus_id(message: Message, state: FSMContext):
    bus_id = (message.text or "").strip()
    if not bus_id:
        await message.answer("❌ ID BUS non valido. Inseriscilo come testo.")
        return

    # Aggiunge una nuova riga: colonna 1 = ID BUS, colonne successive = devices
    try:
        sheet = get_sheet("Matricole")
    except Exception as e:
        logger.exception("Errore aprendo sheet Matricole: %s", e)
        await message.answer("❌ Errore interno: impossibile aprire il foglio Matricole. Contatta l'amministratore.")
        await state.clear()
        return
    ...
    try:
        ...
        # la nuova riga sarà l'ultima
        new_row_index = len(sheet.get_all_values())
    except Exception as e:
        logger.exception("Errore append_row Matricole: %s", e)
        await message.answer("❌ Errore nel salvataggio dell'ID BUS su Google Sheets.")
        await state.clear()
        return

    await state.update_data(row=new_row_index, current_device_index=0)
    await state.set_state(ScanBusForm.waiting_for_device)
    await message.answer(f"✅ ID BUS <b>{bus_id}</b> registrato.\n\nOra invia la FOTO del codice per: <b>{DEVICE_LIST[0]}</b>\n(Se preferisci, puoi anche inviare il codice come testo.)")

@dp.message(ScanBusForm.waiting_for_device, F.photo)
async def process_device_photo(message: Message, state: FSMContext):
    """
    Riceve una foto, prova a decodificare barcode/QR e salva il valore.
    """
    data = await state.get_data()
    if not data:
        await message.answer("❌ Stato non valido. Ripeti con /scan.")
        await state.clear()
        return

    row_index = data.get("row")
    current_index = data.get("current_device_index", 0)
    if row_index is None:
        await message.answer("❌ Stato non valido. Ripeti con /scan.")
        await state.clear()
        return

    # scarica l'immagine come bytes
    try:
        photo = message.photo[-1]
        file = await bot.get_file(photo.file_id)
        file_bytes = await bot.download_file(file.file_path)  # bytes
        img = Image.open(pyio.BytesIO(file_bytes)).convert("RGB")
    except Exception as e:
        logger.exception("Errore download/apertura immagine: %s", e)
        await message.answer("❌ Errore nel ricevere l'immagine. Riprova.")
        return
    ...
    try:
        decoded = decode(img)
        if not decoded:
            await message.answer("❌ Nessun codice rilevato nella foto. Riprova inviando una foto più nitida o invia il codice come testo.")
            return
        code = decoded[0].data.decode("utf-8")
    except Exception as e:
        logger.exception("Errore decodifica immagine: %s", e)
        await message.answer("❌ Errore durante la lettura del codice. Riprova.")
        return

    # salva su sheet (colonna: 1 = ID BUS, quindi device col = 2 + current_index)
    try:
        sheet = get_sheet("Matricole")
        col_index = 2 + current_index  # Excel-style: 1-based; ID BUS=col1 -> device1=col2
        sheet.update_cell(row_index, col_index, code)
    except Exception as e:
        logger.exception("Errore update_cell Matricole: %s", e)
        await message.answer("❌ Errore nel salvataggio su Google Sheets.")
        return

    # rispondi e procedi al prossimo dispositivo
    device_name = DEVICE_LIST[current_index]
    next_index = current_index + 1
    if next_index < len(DEVICE_LIST):
        await state.update_data(current_device_index=next_index)
        await message.answer(f"✅ <b>{device_name}</b> registrata: <code>{code}</code>\n\n📸 Ora invia la FOTO per: <b>{DEVICE_LIST[next_index]}</b> (o invia il codice come testo).", reply_markup=main_kb)
    else:
        await state.clear()
        await message.answer("✅ Tutte le matricole registrate correttamente!\nTorno al menu principale.", reply_markup=main_kb)

@dp.message(ScanBusForm.waiting_for_device)
async def process_device_text(message: Message, state: FSMContext):
    """
    Se l'utente invia testo invece di foto (fallback), salva il testo come codice.
    """
    data = await state.get_data()
    if not data:
        await message.answer("❌ Stato non valido. Ripeti con /scan.")
    ...
    if not code:
        await message.answer("❌ Codice vuoto. Invia una foto o il codice come testo.")
        return

    try:
        sheet = get_sheet("Matricole")
        col_index = 2 + current_index
        sheet.update_cell(row_index, col_index, code)
    except Exception as e:
        logger.exception("Errore update_cell Matricole (text): %s", e)
        await message.answer("❌ Errore nel salvataggio su Google Sheets.")
        return
    ...
    if next_index < len(DEVICE_LIST):
        await state.update_data(current_device_index=next_index)
        await message.answer(f"✅ <b>{device_name}</b> registrata: <code>{code}</code>\n\n📸 Ora invia la FOTO per: <b>{DEVICE_LIST[next_index]}</b> (o invia il codice come testo).")
    else:
        await state.clear()
        await message.answer("✅ Tutte le matricole registrate correttamente!\nTorno al menu principale.", reply_markup=main_kb)
